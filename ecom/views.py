...
def payment_page(request):
    # Assuming you have a Cart model where cart items are stored
    cart = Customer.objects.filter(user=request.user, purchased=False)
    total_amount = sum(item.price for item in cart)  # Calculate total amount

    # Assuming the user has a profile with details
    user_profile = request.user.profile  # Assuming a related Profile model
